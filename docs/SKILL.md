...
dataclaw prep --source all                 # All sources (Claude + Codex + Gemini + OpenCode + OpenClaw + Pi + Hermes)
...
dataclaw prep --source pi                  # Only Pi sessions
...
dataclaw config --source all               # REQUIRED source scope: claude|codex|gemini|opencode|openclaw|pi|hermes|all
...
dataclaw export --source pi --no-push       # Export only Pi sessions
...
- **Source selection is REQUIRED before export** — explicitly set `dataclaw config --source claude|codex|gemini|opencode|openclaw|pi|hermes|all` (or pass `--source ...` on export).
